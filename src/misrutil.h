...
#ifndef _MISR_UTIL_H_
#define _MISR_UTIL_H_
/**
 ...
 * PARAMETERS:
 *	<DTYPE> * originalGrid:	the original input grids (radiance values in one band, latitude or longitude)
 *	<DTYPE> * newGrid:	the output grid (the original input grid after block offseting)
 *	int highResolution: whether the MISR image is high or low resolution
 *		0: low resolution
 *		1: high resolution
 * Output:
 ...
 *
 * NOTE: As a template function, this need to be in header file as a type is dynamically determined. Otherwise link error occurs.
 */
template <typename DTYPE>  // data type <DTYPE> is determined dynamically by caller
void MISRBlockOffset(DTYPE * originalGrid, DTYPE * newGrid, int highResolution) 
{
	int nRowPerBlock;
	int nColPerBlock;
